...
convert_do ()
{
    ...
    for file in *.{mp4,MP4}; do
        if [ -r "$file" ] ; then
            ...
            #nice -n 19 mencoder "tmp_${file}" -oac copy -ovc lavc -lavcopts vcodec=mpeg4:vpass=1:vbitrate=1200 -o "${DONEDIR}"/"${TIMESTAMP}"_"${EXTSTRING}${file%.*}".mp4
            nice -n 19 mencoder "tmp_${file}" -oac copy -ovc lavc -lavcopts vcodec=mpeg4:vpass=1:vbitrate=1200 -o /dev/null
            nice -n 19 mencoder "tmp_${file}" -oac copy -ovc lavc -lavcopts vcodec=mpeg4:vpass=2:vbitrate=1200 -o "${DONEDIR}"/"${TIMESTAMP}"_"${EXTSTRING}${file%.*}".mp4
                rm "tmp_${file}";
            if [ -f "divx2pass.log" ] ; then
                rm divx2pass.log;
            fi
...
if cd "$DONEDIR" ; then
    ...
    for MP4S in *.mp4 ; do
        DIRNAME=`ls --time-style=long-iso -l "$MP4S" | awk '{print $8}' | cut -d- -f1-2`
        if [ ! -d "/home/kk/Videos/family/$DIRNAME" ] ; then
            mkdir -p "/home/kk/Videos/family/$DIRNAME"
        fi
        if [ -f "$MP4S" ] ; then
            cp -au "$MP4S" "/home/kk/Videos/family/$DIRNAME"
        fi
    done
fi
