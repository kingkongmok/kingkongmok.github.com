...
comicDownloadList="/home/kk/Dropbox/home/kk/Downloads/comic/comic.list"
#comicDownloadFinish="/home/kk/Dropbox/home/kk/Downloads/comic/comic.done"
comicDownloadFinish="/tmp/comic.done"
comicDownloadFailure="/tmp/comic.failure"
        ...
        if [ "$FILENAME" != ".zip" ]  ; then
            ...
            if [ "$FailureMode" == "0" ] ; then 
                ...
                if [ ! "$( cat $comicDownloadFailure 2> /dev/null | grep $line )" ]  ; then
                    ...
                    perl -i.bak -nE 'print unless m#'"$1"'#' $comicDownloadFailure
                fi
        ...
        if [ ! -z "$line" ]; then
            ...
            if [ $SIZE -gt 0 ] ; then
                ...
                if  [ "$SIZE" -eq "$localfile_size" ]; then 
                    ...
                    #sed -i.bak \"#$DownloadURL#d\" $comicDownloadList
                ...
                # download the file
                $PROXYCHAINS -q $CURL -s -H "$CURL_HEADER" -C - $DownloadURL -o "$localfile" 
                sleep 3
