...
echo
echo -n "Step 3."
echo "  Please change to /tmp directory to see the REPORT."


RUNTIME=`date |awk '{print $6" "$2" "$3" "$4}'`
REPORTMAINDIR=/tmp/safeeecd
REPORTMAINFILE1=$REPORTMAINDIR/report.`hostname`.`/sbin/ifconfig -a |grep inet |grep -v "127.0.0.1" |awk '{print $2}' |head -n 1`.`date |awk '{print $6"."$2"."$3"."$4}'`
REPORTMAINFILE=`echo $REPORTMAINFILE1|sed -e 's/:/./g'`.log
...
echo "SysPath: `cat /tmp/.PATH`" >>$REPORTMAINFILE
echo "    [ OK ]"
echo "" >>$REPORTMAINFILE
...
echo "**********************************************************************" >>$REPORTMAINFILE
cat /var/log/secure >>$REPORTMAINFILE
echo "_____________________________________________________________________" >>$REPORTMAINFILE
...
echo "**********************************************************************" >>$REPORTMAINFILE
who /var/log/wtmp >>$REPORTMAINFILE
echo "_____________________________________________________________________" >>$REPORTMAINFILE
...
echo "" >>$REPORTMAINFILE
echo "_____________________________________________________________________" >>$REPORTMAINFILE
echo "cron files    " >>$REPORTMAINFILE
echo "_____________________________________________________________________" >>$REPORTMAINFILE
echo "" >>$REPORTMAINFILE
ls -al /etc/crontab >>$REPORTMAINFILE 2>/dev/null
echo "" >>$REPORTMAINFILE
echo "_____________________________________________________________________" >>$REPORTMAINFILE
echo "log files    " >>$REPORTMAINFILE
echo "_____________________________________________________________________" >>$REPORTMAINFILE
echo "" >>$REPORTMAINFILE
ls -al /var/log/secure >>$REPORTMAINFILE 2>/dev/null
echo "" >>$REPORTMAINFILE
echo "" >>$REPORTMAINFILE
echo "    [ OK ]"
