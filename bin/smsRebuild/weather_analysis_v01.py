#!/usr/bin/env python
# -*- coding:UTF-8 -*-
'''weather 站点性能分析,思路:把接口分为三组,分为格式化到字典里,字典格式为:
   statisDic{'interfaceName':{'explain':'','total':30,'lt50':3,'lt100':10 .....}}'''

import os,sys,re
import time
logTime = time.strftime('%Y-%m-%d',time.localtime(time.time()-24*60*60))
logName = ['/home/logs/smsmw/172.16.200.2/weather/monitoring.log.'+logTime,\
           '/home/logs/smsmw/172.16.200.8/weather/monitoring.log.'+logTime,\
           '/home/logs/smsmw/172.16.200.9/weather/monitoring.log.'+logTime]
...
if os.path.isfile('/tmp/weather_analysis.html'):
    os.remove("/tmp/weather_analysis.html")
...
#格式化输出
tempLog = open('/tmp/weather_analysis.html','a')
...
tempLog.write('</table>')
tempLog.close()
# os.system('/home/appSys/smsRebuild/sbin/weather_analysis_py.sh')
