...
foreach my $line ( @lines ){
    ...
    my $message = `/usr/bin/proxychains4 -q curl -s $line`; 
    if ($message){
        if ( $message =~ /download_filename\"\>(.*?.zip)\<\/p\>
            ...
            if (  exists $downloaded_url{$DownloadUrl} && ! $opt_f){ 
                ...
                #-------------------------------------------------------------------------------
                my $httpcode = `/usr/bin/proxychains4 -q curl -s -o /dev/null -I -w '%{http_code}' \"$DownloadUrl\"`;
                sleep 5;
                #-------------------------------------------------------------------------------
                #  download comic with curl/aria if http coder = 200
                #-------------------------------------------------------------------------------
                if ( $httpcode == 200 ) {
                    $result = system("/usr/bin/proxychains4 -q curl -C -  \"$DownloadUrl\"" . 
