#!/usr/bin/env python
# -*- coding:UTF-8 -*-
'''Setting 站点性能分析,思路:把接口分为三组,分为格式化到字典里,字典格式为:
   statisDic{'interfaceName':{'explain':'','total':30,'lt50':3,'lt100':10 .....}}'''

import os,sys,re
import time
logTime = time.strftime('%Y-%m-%d',time.localtime(time.time()-24*60*60))
logName = ['/home/logs/smsmw/172.16.200.2/setting/monitoring.log.'+logTime,'/home/logs/smsmw/172.16.200.8/setting/monitoring.log.'+logTime,\
		  '/home/logs/smsmw/172.16.200.9/setting/monitoring.log.'+logTime]
#logName = ['/home/logs/smsmw/172.16.200.2/setting/monitoring.log.'+logTime]
setDic = {'unified:getUnifiedPositionContent':'同一位置','user:getInfoCenter':'信息中心','user:getMailNotify':'邮件到达提醒',\
		  'user:getMainData':'邮箱属性','umc:getArtifact':'用管中心凭证','bill:getTypeList':'账单信息',\
		  'user:setUserConfigInfo':'用户配置信息','info:getInfoSet':'欢迎页信息'}
...
if os.path.isfile('/tmp/setting_analysis.html'):
	os.remove("/tmp/setting_analysis.html")
...
#格式化输出
tempLog = open('/tmp/setting_analysis.html','a')
...
tempLog.write('</table>')
tempLog.close()
os.system('/home/appSys/smsRebuild/sbin/setting_analysis_py.sh')
