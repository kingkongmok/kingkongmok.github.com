...
find  /home/logs/*_mmlogs/crontabLog/nginx/ -name \*log -mtime -1 | xargs tar Pczf /tmp/pvAnalyze_today.tar.gz 

echo -e "\n\n<H1>================== logs today below==================</H1>\n\n" >> /tmp/nginx_status_now.txt
for i in /tmp/nginxPVHourly.png  /tmp/nginxPVToday.png /tmp/nginxPVPerServerHourly.png /tmp/nginxPVPerServerToday.png; do echo -en "<img src=\"data:image/png;base64," `base64 $i` "\"\>\n" ; done >> /tmp/nginx_status_now.txt
