...
printflush $P qq[
        ...
        set title "All Tomcat $yesterday 2XX minutely" font "/usr/share/fonts/dejavu-lgc/DejaVuLGCSansMono-Bold.ttf, 20"
        set xdata time
        set timefmt "%H:%M"
        set format x "%H:%M"
        set grid
...
`cp $N "/opt/mmSdk/tmp/$yesterday-200.txt"` ;
`cp "/tmp/tomcat200.png" "/opt/mmSdk/tmp/$yesterday-200.png"` ;
