...
if [ `ps -ef|grep vmmemctl|grep -v grep|wc -l` -gt 0 ]
        ...
        if [ $? -eq 0 ]; then
                sudo /sbin/multipath -ll|grep size|awk '{print $1}'|cut -f2 -d= > /tmp/multipath01.tmp
                multipath_M=`cat /tmp/multipath01.tmp |grep M|cut -f1 -dM|awk '{sum += $1};END {print sum/1024 }'`
                multipath_G=`cat /tmp/multipath01.tmp |grep G|cut -f1 -dG|awk '{sum += $1};END {print sum }'`
                multipath_T=`cat /tmp/multipath01.tmp |grep T|cut -f1 -dT|awk '{sum += $1};END {print sum*1024 }'`
                rm -rf /tmp/multipath01.tmp
...
if [ `ps -ef|grep ora_lgwr|grep -v grep|wc -l` -gt 0 ]
        then
        sudo su - oracle <<EOF > /tmp/ora_log01.tmp
        sqlplus / as sysdba 
        select sum(bytes)/1024/1024/1024 from dba_data_files;
        select sum(bytes)/1024/1024/1024 from dba_segments;
        exit; 
EOF
        ORA_RAW=`cat /tmp/ora_log01.tmp |grep -PA 1 "\-----"|sed -n '2p'`
        ORA_DATA=`cat /tmp/ora_log01.tmp |grep -PA 1 "\-----"|sed -n '5p'`
        rm -rf /tmp/ora_log01.tmp
else
        ORA_RAW=0
        ORA_DATA=0
fi
...
let disk_total=${DISK%.*}*1024*1024*1024
echo $disk_total
