...
my $pathcount = {
    ...
    '/phpinfo.php' => 0,
    '/phpinfotest.php' => 0,
    '/zabbix/dashboard.php' => 0,
    '/zabbix/chart2.php' => 0,
    '/zabbix/jsLoader.php' => 0,
    '/zabbix/charts.php' => 0,
...
my $l = $_;
    ...
    foreach my $cfg (@$CONFIG) {
      ...
      my $splitedPath;
      if ( $path =~ /\?/ ) {
         $splitedPath = +(split /\?/, $path, 2)[0];
      } else {
          $splitedPath = $path;
      }
      # my $splitedPath  =  $path =~ /\?/  ? +(split /\?/, $path)[0] : $path;
      $r->{pathcount}->{defined $r->{pathcount}->{$splitedPath} ? $splitedPath : 'other'} += 1;
...
foreach my $cfg (@$CONFIG) {
  ...
  for my $path (keys %{$r->{pathcount}}) {
    sendstat("path_$path", $pathcount->{$path}, $cfg);
  }
}
